...
console.log(oA === oB); // 解析： 在第一次调用构造函数时利用闭包存储一个instance实例， 以后的调用直接返回instance.
// ES6写法
...
console.log(oA === oB); // static 关键字解释：类相当于实例的原型， 所有在类中定义的方法， 都会被实例继承。 如果在一个方法前， 加上static关键字， 就表示该方法不会被实例继承， 而是直接通过类来调用， 这就称为“ 静态方法”。
