...
var p = new PointClass(1, 2); // 结论：ES5的构造函数 对应 ES6类的构造方法

console.log(typeof PointClass); // function --> 类的数据类型就是函数

console.log(PointClass === PointClass.prototype.constructor); // true --> 类本身指向构造函数
...
console.log(PointClass2.prototype.constructor === PointClass2); // true
// true
// 类内部定义方法不可枚举，与 ES5 行为不一致

console.log(Object.keys(Point.prototype));
console.log(Object.keys(PointClass2.prototype)); // [ 'toString' ] --> 方法对象的原型上定义的函数可被枚举到
// [] --> 类内部定义方法不可枚举，与 ES5 行为不一致

console.log(Object.getOwnPropertyNames(Point.prototype));
console.log(Object.getOwnPropertyNames(PointClass2.prototype)); // [ 'constructor', 'toString' ]
// [ 'constructor' ]

console.log(Object.keys(Point));
console.log(Object.keys(PointClass2)); // 这是什么??
// []
// [ 'toString', 'toValue' ]
...
/**
 * name属性
 * 因为 ES6 class 只是对 ES5 构造函数的一层封装
 * 所以函数很多特性都被类继承
 */
...
/**
 * this 指向
 * 类的方法内部如果有 this, 默认指向类的实例
 * 但是将方法单独提出来调用可能会丢失 this
 */

class Logger {
  printName() {
    let name = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'there';
    this.print("Hello ".concat(name));
  }
...
const {
  printName
} = logger; // printName(); // TypeError: Cannot read property 'print' of undefined
// this 指向运行时所在环境

/**
 * 解决 this 丢失方法 1 —— 绑定 this
 */
