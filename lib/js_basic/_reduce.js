...
// acc: 1
// _reduce.js:41 cur: 2
// _reduce.js:42 acc + cur: 3
// _reduce.js:40 acc: 3
// _reduce.js:41 cur: 3
// _reduce.js:42 acc + cur: 6
// _reduce.js:40 acc: 6
// _reduce.js:41 cur: 4
// _reduce.js:42 acc + cur: 10
...
function mapBeforeReduce() {
  ...
  const data3 = []; // const reducer = (acc, cur) => Math.max(acc.x, cur.x);
  // console.log(data1.reduce(reducer))
  // console.log(data2.reduce(reducer))
  // console.log(data3.reduce(reducer))
  //   24
  // _reduce.js:71 {x: 22}
  // _reduce.js:72 Uncaught TypeError: Reduce of empty array with no initial value
  ...
  console.log(data3.map(el => el.x).reduce(advancedReducer, -Infinity)); //   24
  // _reduce.js:83 22
  // _reduce.js:84 -Infinity

  /**
   * 拿出对象中的某个属性做最大值筛选
   * 可以对复杂结构的数据做计算处理
   * 
   * 传入默认初始值-Infinity主要是兼容空数组不报错
   */
  ...
  console.log(data3Result); //   (2) [22, 24]
  // _reduce.js:101 [22]
  // _reduce.js:103 []
} // mapBeforeReduce()
...
function groupByProp() {
  ...
  console.log(result); // groupedPeople is:
  // { 
  //   20: [
  //     { name: 'Max', age: 20 }, 
  //     { name: 'Jane', age: 20 }
